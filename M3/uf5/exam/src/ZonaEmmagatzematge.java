...
public class ZonaEmmagatzematge {
    ...
    public void ubicar(Container container) throws Exception {
        ...
        throw new Exception("No es pot ubica el container amb id: " + container.getIdentificador() + " per qué no hi ha cap espai disponible");
    }

    public Container retirar(String identificador) throws Exception {
        Ubicacio ubicacio = findUbicacio(identificador);
        if (ubicacio == null) {
            throw new Exception("No s'ha trobat cap contenidor amb id: " + identificador);
        }
        while (!identificador.equals(ubicacio.superior().getIdentificador())) {
            ubicar(ubicacio.superior());
            ubicacio.treure();
        }
        if (identificador.equals(ubicacio.superior().getIdentificador())) {
            return ubicacio.treure();
        }
        return null;
    }
