...
public class InMemoryArticleRepository implements ArticleRepository {
    ...
    public Article getArticleByReference(String reference) {
        for (Article article : listOfArticles) {
            if (article != null && article.getReference() != null && article.getReference().equals(reference)) {
                return article;
            }
        }
        throw new IllegalArgumentException("No articles with reference=" + reference + " found");
    }
}
